...
export enum Faction {
    ...
    // Content hook, no clients yet (design doc): declared so future
    // registrations have a home, never assigned a client.
    BASALT_COURTS = "The Basalt Courts",
    // Went live at the opposition-system merge: De Koninklijke
    // Wetenschappelijke Helrader Maatschappij commissions specimen culls.
 ...
 *  Union was originally registered rivalry-target-only ("Labor does not
 *  commission mercenaries"), but the opposition-system merge ships the
 *  one deliberate exception: the Amalgamated Chapters retainer, in which
 *  the Union buys the Company's services against the Company's own
 *  overseers precisely because our billable hours have no politics.
 *  Completing it raises Union standing and, via the rivalry pairs,
 *  strains the Barons and the Board of Overseers — working as intended. */
export const CLIENT_FACTIONS: Record<string, Faction> = {
    ...
    // The exception that proves "labor does not commission mercenaries" —
    // see this map's doc comment.
    "The Stoker's Union, Amalgamated Chapters": Faction.STOKERS_UNION,

    // First Clogger client (the enum's content hook, now live): specimen
    // -collection culls along the Skeeterwisp Line, delta-fauna opposition.
    "De Koninklijke Wetenschappelijke Helrader Maatschappij": Faction.CLOGGERS,
