...
describe('CLIENT_FACTIONS sanity', () => {
    ...
    it('maps exactly one client to the Stokers\' Union — the Amalgamated Chapters retainer ' +
        '(the deliberate exception to "labor commissions no mercenaries"; see CLIENT_FACTIONS\' doc comment)', () => {
        const unionClients = Object.entries(CLIENT_FACTIONS)
            .filter(([, f]) => f === Faction.STOKERS_UNION)
            .map(([client]) => client);
        expect(unionClients).toEqual(["The Stoker's Union, Amalgamated Chapters"]);
    });
});
